...
void removeChannel(Channel *c){
	...
	semctl((*c).p_lock, 0, IPC_RMID, NULL);
	semctl((*c).available, 0, IPC_RMID, NULL);
	semctl((*c).taken, 0, IPC_RMID, NULL);
...
void semWait(int semid){
	...
	semid = semget(semid, nsems, semflg);
...
void put(Channel *c, char *ptr, int len){
	...
	printf("FINISHED PUT CHAR Length: %d",len);
}
