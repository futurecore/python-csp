...
@process
def ParcalculateRowColumnProduct(cout, A, row, B, col, width, height):
    i = row
    j = col
    res = []
...
class Matrix():
    def __init__(self, h, k):
        ...
        def Multiply(self, mb):
            ...
            a = self.matrix
            if len(a[0]) != len(b):
                raise Exception() # FIXME: Use a sensible exception class
                return
        ...
        def ParMultiply(self,mb,gran):
            ...
            for i in range((len(a)/gran)) :
                for j in range((len(b[0])/gran)):
                    ch = Channel()
                    chnls.append(ch);
                    procs.append(ParcalculateRowColumnProduct(ch,a,(i*gran),b,(j*gran),
                                                              gran,
                                                              gran))            
