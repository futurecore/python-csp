...
"""
Guards and channel types for the process version of the CSP library.
...
class Channel(Guard):
    ...
    def __init__(self):
        ...
        _chdebug('Channel created:', self.name)
        return
    ...
    def get(self):
        ...
        while True:
            ...
            _chdebug('Read from OS pipe')
            if not sval:
                break
            data.append(sval)
#            _chdebug('Pipe got data: %i, %s' % (len(sval), sval))
            if len(sval) < _BUFFSIZE:
                break
        _chdebug('Left read loop')
        _chdebug('About to unmarshall this data:', ''.join(data)) 
        obj = None if data == [] else mypickle.loads(''.join(data))
        _chdebug('mypickle library has unmarshalled data.')
        return obj
    ...
    def checkpoison(self):
        with self._plock:
            if self._poisoned.value == Channel.TRUE:
                _chdebug(self.name, 'is poisoned. Raising ChannelPoison()')
                raise ChannelPoison()
