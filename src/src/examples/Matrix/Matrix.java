...
public class Matrix {
	...
	public Matrix ParMultiplyMatrix(Matrix Mb, int gran) throws Exception {
		...
		JavaCspProcess[] procs = new JavaCspProcess[matrix.length/gran];
		for (int i = 0; i < matrix.length / gran; i++) {
			for (int j = 0; j < matrix[0].length / gran; j++) {
				procs[j] = new RCProduct(a, i * gran, b, j * gran, res,
						gran, gran);
	...
	public class RCProduct extends JavaCspProcess {
		 ...
		 * @param res
		 */
		public RCProduct(float[][] a, int row, float[][] b, int col,
				Matrix res, int width, int height) {
			super();
			this.A = a;
			this.row = row;
			this.B = b;
